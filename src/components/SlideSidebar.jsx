...
const SlideSidebar = () => {
  ...
  return (
    <div className="h-dvh w-60 bg-neutral-50 shadow-sm">
        <div className="ml-[6px] md:ml-[22px] mt-6 flex flex-col gap-6 mr-[6px] md:mr-[22px]">
           {sidebarLinks.map((section)=>(
             <div key={section.title}>
                <div className="flex gap-1.5 md:gap-2 items-center "> 
                  <div>{section.icon}</div>
                  <h2 className="font-lato text-base">{section.title}</h2>
                </div>
                

                <div className="flex flex-col gap-1 mt-3 pl-1 md:pl-0">
               {section.content.map((item)=>(
              <a key={item.heading} className="flex items-center justify-start gap-[9px] md:gap-3">
                    <div>{item.icon}</div>
